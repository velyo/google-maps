...
// ------------------------------------------------------------------------------------------------
// Copyright (C) Velyo Ivanov.
// ------------------------------------------------------------------------------------------------
// GoogleMap.debug.js
// GoogleMap Control JavaScript library (debug).
//
// Assembly:    Velyo.Google.Maps
// Version:     7.0.0.0
// Project:     https://github.com/velyo/google-maps
// Demo:        http://googlemap.artembg.com
// Author:      Velyo Ivanov - velio.ivanov@gmail.com
//              http://velyo.net
// License:     Microsoft Permissive License (Ms-PL) v1.1
//              https://github.com/velyo/google-maps/blob/master/LICENSE
// API:         http://code.google.com/apis/maps/

Type.registerNamespace("Velyo.Google.Maps"); 

// Map class
Velyo.Google.Maps.Map = function (element) {
    /// <summary>This class represents the client GoogleMap control object.</summary>
    Velyo.Google.Maps.Map.initializeBase(this, [element]);
}; 

Velyo.Google.Maps.Map.prototype = {

    initialize: function () {
        Velyo.Google.Maps.Map.callBaseMethod(this, 'initialize');
        this.create();
    },

    dispose: function () {
        this.detachEvents();
        Velyo.Google.Maps.Map.callBaseMethod(this, 'dispose');
    }
};
Velyo.Google.Maps.Map.registerClass("Velyo.Google.Maps.Map", Sys.UI.Control);
...
(function (proto) {
    ...
    proto.create = function (location) {
        ...
        if (!location) {
            if (this.center)
                location = Velyo.Google.Maps.Convert.toLatLng(this.center);
        }
        else {
        ...
        if (location || this.bounds) {
            var options = {
                center: location,
                mapTypeId: Velyo.Google.Maps.Convert.toMapTypeId(this.mapType),
                zoom: this.zoom,
            ...
            if (this.searchBoxControl) {
                ...
                input.onkeypress = function (e) {
                    if (e.keyCode == 13) { // Detect Enter
                        ...
                        Velyo.Google.Maps.Geocoding.getResults(options, function (results) {
            ...
            if (this.enableReverseGeocoding) {
                ...
                Velyo.Google.Maps.Geocoding.getAddress(options,
            ...
            Velyo.Google.Maps.Geocoding.getLocation(options, Function.createDelegate(this, this.create));
        }
    };
...
})(Velyo.Google.Maps.Map);

// utilities //////////////////////////////////////////////////////////////////

// convert
Velyo.Google.Maps.Convert = (function () {
...
// geocoding
Velyo.Google.Maps.Geocoding = (function () {
        ...
        geocoder.geocode(request, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                try {
                    ...
                    Velyo.Google.Maps.Log.error(ex);
                }
            }
            else {
                Velyo.Google.Maps.Log.error("Reverse location geocoding failed with status: " + status);
            }
        });
    }
    ...
    function handleAddressError(status, options, callback) {
        ...
        if (options.defaultAddress) {
            Velyo.Google.Maps.Log.warn(status + " ... applying default address.");
            ...
            Velyo.Google.Maps.Log.error("Address geocoding failed with status: " + status + "\nDefaultAddress is not set and map was left empty!");
        }
    }
...
// log
Velyo.Google.Maps.Log = (function () {
...
Velyo.Google.Maps.Worker = (function () {
    var tasks = [];
    var timer = null;
...
// utility functions
// object merging
Velyo.Google.Maps.merge = function (obj1, obj2) {
    var result = {};
    if (obj1)
