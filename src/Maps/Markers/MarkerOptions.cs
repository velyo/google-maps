...
namespace Velyo.Google.Maps
{
    ...
    {
        private MarkerImage _icon;
        private LatLng _position;
        private MarkerImage _shadow;
        ...
        {
            get { return _icon ?? (_icon = new MarkerImage()); }
            set { _icon = value; }
        }
        ...
        public bool? RaiseOnDrag { get; set; } = true;
        ...
        {
            get { return _shadow ?? (_shadow = new MarkerImage()); }
            set { _shadow = value; }
        }
        ...
        {
            get { return _shape ?? (_shape = new MarkerShape()); }
            set { _shape = value; }
        }
        ...
        public virtual IDictionary<string, object> ToScriptData()
        {
            ...
            if (Animation.HasValue) result["animation"] = Animation.Value;
            if (Clickable.HasValue) result["clickable"] = Clickable.Value;
            if (Cursor != null) result["cursor"] = Cursor;
            if (Draggable.HasValue) result["draggable"] = Draggable.Value;
            if (Flat.HasValue) result["flat"] = Flat.Value;
            if (_icon != null) result["icon"] = _icon.ToScriptData();
            if (Optimized.HasValue) result["optimized"] = Optimized.Value;
            if (_position != null) result["position"] = _position.ToScriptData();
            if (RaiseOnDrag.HasValue) result["raiseOnDrag"] = RaiseOnDrag.Value;
            if (_shadow != null) result["shadow"] = _shadow.ToScriptData();
            if (_shape != null) result["shape"] = _shape.ToScriptData();
            if (Title != null) result["title"] = Title;
            if (Visible.HasValue) result["visible"] = Visible.Value;
            if (ZIndex.HasValue) result["zIndex"] = ZIndex.Value;
