...
Velyo.Google.Maps.MarkersBehavior = function (element) {
    Velyo.Google.Maps.MarkersBehavior.initializeBase(this, [element]);
};

Velyo.Google.Maps.MarkersBehavior.prototype = {
    initialize: function () {
        Velyo.Google.Maps.MarkersBehavior.callBaseMethod(this, 'initialize');
        Velyo.Google.Maps.Worker.queue(Function.createDelegate(this, this._attach));
    },
    dispose: function () {
        this._detach();
        Velyo.Google.Maps.MarkersBehavior.callBaseMethod(this, 'dispose');
    }
};
...
(function (proto) {
    ...
    proto.create = function () {
        ...
        if (this.markerOptions) {
            ...
            for (var i = 0; i < this.markerOptions.length; i++) {
                var marker = Velyo.Google.Maps.merge(this.groupOptions, this.markerOptions[i]);
                marker.map = this.map;
                marker.index = i;
                ...
                if (marker.position) {
                    ...
                    Velyo.Google.Maps.Geocoding.getLocation(options, delegate);
                }
            }
        }
    };
    ...
    function setMarker(position, marker) {
        ...
        if (marker) {
            ...
            if (marker.info) {
                ginfo = new google.maps.InfoWindow(Velyo.Google.Maps.merge({ content: marker.info }, this.infoOptions));
                marker.info = ginfo;
...
})(Velyo.Google.Maps.MarkersBehavior);

Velyo.Google.Maps.MarkersBehavior.registerClass('Velyo.Google.Maps.MarkersBehavior', Sys.UI.Behavior);