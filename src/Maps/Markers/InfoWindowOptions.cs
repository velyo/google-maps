...
namespace Velyo.Google.Maps
{
    ...
    public class InfoWindowOptions : IScriptDataConverter
    {
        ...
        public IDictionary<string, object> ToScriptData()
        {
            ...
            if (DisableAutoPan.HasValue) data["disableAutoPan"] = DisableAutoPan.Value;
            if (MaxWidth.HasValue) data["maxWidth"] = MaxWidth.Value;
            if (_pixelOffset != null) data["pixelOffset"] = _pixelOffset.ToScriptData();
            if (_position != null) data["position"] = _position.ToScriptData();
            if (ZIndex.HasValue) data["zIndex"] = ZIndex.Value;
