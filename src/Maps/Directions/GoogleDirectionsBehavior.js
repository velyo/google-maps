...
Velyo.Google.Maps.DirectionsBehavior = function (element) {
    Velyo.Google.Maps.DirectionsBehavior.initializeBase(this, [element]);
}

Velyo.Google.Maps.DirectionsBehavior.prototype = {
    initialize: function () {
        Velyo.Google.Maps.DirectionsBehavior.callBaseMethod(this, 'initialize');
        Velyo.Google.Maps.Worker.queue(Function.createDelegate(this, this._attach));
    },
    dispose: function () {
        this._detach();
        Velyo.Google.Maps.DirectionsBehavior.callBaseMethod(this, 'dispose');
    }
};
...
(function (proto) {
    ...
    proto.create = function () {
        ...
        var request = {
            ...
            origin      : this.origin,
            travelMode  : eval('google.maps.TravelMode.' + this.travelMode)
        };
        ...
        if (request.origin && request.destination) {
            ...
            if (!this.origin) Velyo.Google.Maps.Log.warn("GoogleDirections: origin value is missing!");
            if (!this.destination) Velyo.Google.Maps.Log.warn("GoogleDirections: destination value is missing!");
        }
    };
    ...
    function handleError(status) {
        Velyo.Google.Maps.Log.error("Directions routing failed with status: " + status);
    }
...
Velyo.Google.Maps.DirectionsBehavior.raiseServerChanged = function (sender) {
...
Velyo.Google.Maps.DirectionsBehavior.registerClass('Velyo.Google.Maps.DirectionsBehavior', Sys.UI.Behavior);
